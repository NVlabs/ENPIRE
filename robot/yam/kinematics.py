...
def _rpy_display_to_quat_xyzw(rpy_display: Any) -> np.ndarray:
    ...
    if rpy.size != 3:
        raise ValueError(
            f"Expected RPY with 3 values, got shape {np.asarray(rpy_display).shape}"
        )
...
def _quat_xyzw_to_rpy_display(q_xyzw: Any) -> np.ndarray:
    ...
    if q.size != 4:
        raise ValueError(
            f"Expected quaternion with 4 values, got shape {np.asarray(q_xyzw).shape}"
        )
...
class YamKinematics:
    ...
    def forward_kinematics(
        ...
        left_ee_pose = self.configuration.get_transform_frame_to_world(
            "left_grasp_site", "site"
        )
        right_ee_pose = self.configuration.get_transform_frame_to_world(
            "right_grasp_site", "site"
        )
    ...
    def inverse_kinematics(
        ...
        # Update targets
        T_wt_left = mink.SE3.from_rotation_and_translation(
            mink.SO3(wxyz=left_ee_quat), left_ee_pos
        )
    ...
    def inverse_kinematics_full(
        ...
        if left_active:
            ...
            self.left_end_effector_task.set_target(
                mink.SE3.from_rotation_and_translation(
                    mink.SO3(wxyz=left_wxyz), left_ee_pos
                )
            )
        ...
        if right_active:
            ...
            self.right_end_effector_task.set_target(
                mink.SE3.from_rotation_and_translation(
                    mink.SO3(wxyz=right_wxyz), right_ee_pos
                )
            )
    ...
    def inverse_kinematics_orientation_only(
        ...
        if left_ee_quat_xyzw is not None:
            left_pose = self.configuration.get_transform_frame_to_world(
                "left_grasp_site", "site"
            )
            left_wxyz = left_ee_quat_xyzw[[3, 0, 1, 2]]
        ...
        if right_ee_quat_xyzw is not None:
            right_pose = self.configuration.get_transform_frame_to_world(
                "right_grasp_site", "site"
            )
            right_wxyz = right_ee_quat_xyzw[[3, 0, 1, 2]]
        ...
        if tasks:
            ...
            for _ in range(max_iters):
                vel = mink.solve_ik(
                    self.configuration, tasks, dt, solver, damping, limits=limits
                )
                self.configuration.integrate_inplace(vel, dt)
                if all(
                    np.linalg.norm(t.compute_error(self.configuration)) <= err_threshold
