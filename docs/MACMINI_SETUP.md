...
- **Defaults `--rl-host` to `192.168.50.2`** (direct Ethernet to GPU box) (`launch_sim.sh:83-84`)
...
The cap_ui dev server auto-opens in the browser once ready. The SSH tunnels forward SAM3 and BundleSDF ports from `lecar-s1` (`launch_sim.sh:124-129`).
...
|---|---|---|
| Direct Ethernet (current) | `192.168.50.2` | ~2ms RTT |
| Lab LAN (old) | `172.26.218.216` | ~5-10ms RTT |
...
**Key remote scripts:**
- `tmux/remote_serving/launch_lecar_s1.sh` — Starts SAM3, AnyGrasp, BundleSDF, cuRobo on S1
...
**`scripts/sync_grip_logs.sh`** — Syncs HIL gripper debug logs from `lecar-yam-deploy` to the local Mac Mini. Runs `rsync` every 2 seconds:
