# fridge_handle_grasp.py — skill library, append-only
...
def fridge_handle_grasp_v1(
    ...
    print(f"Close dir:{[round(float(x), 3) for x in pull_dir]}")
    ...
    print(
        ...
        f" close_xy={[round(float(x), 3) for x in pull_dir_xy]}"
    ...
    if not move_checked(
        ...
        return False, {"reason": "failed to reach fridge handle"}
