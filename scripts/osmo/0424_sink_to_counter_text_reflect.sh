#!/usr/bin/env bash
# pick_place_sink_to_counter — text reflection (skill_reflection disabled)
SESSION="${TMUX_SESSION:-osmo-eval}"
WINDOW="sink_counter_text"
...
PROJECT_DIR="/mnt/amlfs-02/shared/wenli_vla_ft/forge"
...
if ! tmux list-windows -t "$SESSION" -F '#{window_name}' | grep -qx "$PROVIDER_WINDOW"; then
    ...
    tmux send-keys -t "$SESSION:$PROVIDER_WINDOW" "UV_PROJECT_ENVIRONMENT=/mnt/amlfs-02/shared/wenli_vla_ft/python_envs/forge PYTHONHASHSEED=42 \
...
tmux send-keys -t "$SESSION:$WINDOW" "while true; do
    MUJOCO_GL=egl UV_PROJECT_ENVIRONMENT=/mnt/amlfs-02/shared/wenli_vla_ft/python_envs/forge PYTHONHASHSEED=42 \
        ...
        execution.seeds_per_gpu=1 oracle_api=true skill_reflection.enabled=false \
