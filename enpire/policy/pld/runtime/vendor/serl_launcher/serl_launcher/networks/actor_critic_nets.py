    ...
    def __call__(
        ...
        if self.encoder is None:
            ...
            obs_enc = self.encoder(
                observations, train=train, stop_gradient=self.freeze_encoder
            )
...
class Policy(nn.Module):
    # Policy network w/ base action input
    encoder: Optional[nn.Module]
    network: nn.Module
    action_dim: int
    init_final: Optional[float] = None
    ...
    def __call__(
        ...
        obs_concat = jnp.concatenate(
            [obs_enc, base_actions], axis=-1
        )  # (batch_size, obs_dim + action_dim)
        outputs = self.network(obs_concat, train=train)
...
class PolicyCalQL(nn.Module):
    """
    Policy network for CalQL/CQL that matches the original architecture.
    This version accepts base_actions for compatibility but ignores it.
    """
    ...
    def __call__(
        self,
        observations: jnp.ndarray,
        base_actions: Optional[jnp.ndarray] = None,  # Accept but ignore for compatibility
        temperature: float = 1.0,
        train: bool = False,
        non_squash_distribution: bool = False,
    ) -> distrax.Distribution:
        # Ignore base_actions - this network doesn't use them
        if self.encoder is None:
            obs_enc = observations
        else:
        ...
        if self.fixed_std is None:
            if self.std_parameterization == "exp":
                ...
                stds = jnp.exp(log_stds)
            elif self.std_parameterization == "fixed":
                stds = self.fixed_std
            else:
                raise ValueError(f"Invalid std_parameterization: {self.std_parameterization}")
        else:
        ...
        if self.tanh_squash_distribution and not non_squash_distribution:
            ...
            if self.tanh_squash_distribution and non_squash_distribution:
                logging.warning(
                    "Making non squashed distribution in squashed actor. remember this is just for debugging initialization!"
                )
            distribution = distrax.MultivariateNormalDiag(
                loc=means,
                scale_diag=stds,
