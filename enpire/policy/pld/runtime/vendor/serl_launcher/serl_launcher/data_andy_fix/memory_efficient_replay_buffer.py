...
from serl_launcher.data_andy_fix.dataset import DatasetDict, _sample
from serl_launcher.data_andy_fix.replay_buffer import ReplayBuffer
...
class MemoryEfficientReplayBuffer(ReplayBuffer):
    def __init__(
        ...
        super().__init__(
            ...
            include_next_base_action=include_next_base_action,
            discount=discount,
    ...
    def insert(self, data_dict: DatasetDict):
        if self._insert_index == 0 and self._capacity == len(self) and not self._first:
            ...
            for indx in indxs:
                ...
                # Unfreeze the element since insert expects a mutable dict
    ...
    def _cleanup_overwritten_trajectories(self):
        """Override to also update _is_correct_index when trajectories are removed."""
        if not hasattr(self, "_allow_idxs"):
            return

        # Call parent method to clean up trajectories
        super()._cleanup_overwritten_trajectories()

        # After cleanup, ensure _is_correct_index is consistent with allowed indices
        # Some indices in _allow_idxs might be marked as incorrect due to frame stacking
        # We need to respect the frame stacking logic, so we filter _allow_idxs
        corrected_allow_idxs = []
        for idx in self._allow_idxs:
            if self._is_correct_index[idx]:
                corrected_allow_idxs.append(idx)

        self._allow_idxs = corrected_allow_idxs
