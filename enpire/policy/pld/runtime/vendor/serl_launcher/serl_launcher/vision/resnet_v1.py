...
class SpatialSoftmax(nn.Module):
    ...
    def __call__(self, features):
        ...
        expected_x = jnp.sum(self.pos_x * softmax_attention, axis=2, keepdims=True).reshape(
            batch_size, num_featuremaps
        )
        expected_y = jnp.sum(self.pos_y * softmax_attention, axis=2, keepdims=True).reshape(
            batch_size, num_featuremaps
        )
...
class ResNetEncoder(nn.Module):
    ...
    def __call__(
        ...
        if self.pooling_method == "spatial_learned_embeddings":
            ...
            x = SpatialSoftmax(height, width, channel, pos_x, pos_y, self.softmax_temperature)(
                x
            )
    ...
    def __call__(
        ...
        if encode:
            x = self.pretrained_encoder(
                x,
                train=train,
                stop_gradient=bool(self.freeze_backbone or stop_gradient),
            )
