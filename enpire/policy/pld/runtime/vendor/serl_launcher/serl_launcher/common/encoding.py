...
class EncodingWrapper(nn.Module):
    ...
    def __call__(
        ...
        if self.use_proprio:
            ...
            if self.enable_stacking:
                # Combine stacking and channels into a single dimension
                if len(state.shape) == 2 and not _batched_input:
                    # Unbatched with stacking: state is (T, C) → flatten
                    state = rearrange(state, "T C -> (T C)")
                    encoded = encoded.reshape(-1)
                elif len(state.shape) == 2 and _batched_input:
                    # Batched without stacking dim on state: (B, C)
                    # Flatten spatial dims of encoded for concat
