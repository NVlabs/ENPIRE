...
(`tmux/remote_serving/launch_remote_gpu.sh:27`, `tmux/table_bussing/launch_table_bussing_remote.sh:60`).
...
|---------|-------------|-----|
| `serve_sam3` | `enpire-remote-serving` pane | `launch_remote_gpu.sh:214` |
| `serve_anygrasp` | `enpire-remote-serving` pane | `launch_remote_gpu.sh:221` |
| `serve_bundlesdf` | `enpire-remote-serving` pane | `launch_remote_gpu.sh:224` |
| `serve_portal_motion_planner` (cuRobo) | `enpire-remote-serving` pane | `launch_remote_gpu.sh:228` |
...
- **S1 (remote GPU)**: use `/usr0/<user>/tmp` because the home filesystem is space constrained
...
- `/usr0/<user>/tmp/huggingface/hub/models--facebook--sam3`
- `/usr0/<user>/tmp/huggingface/hub/models--facebook--sam2.1-hiera-large`
...
2. Check if remote tmux session `enpire-remote-serving` exists and is healthy
...
### S1 launcher CLI flags (`launch_remote_gpu.sh`)

Services are positional args (default: `sam3 anygrasp bundlesdf curobo`):
```bash
bash tmux/remote_serving/launch_remote_gpu.sh sam3 bundlesdf  # only these two
```
...
### Remote session on S1: `enpire-remote-serving`
