...
```bash
/mnt/shared/<user>/forge-code
```

The original reference repository was:

```bash
/mnt/shared/<user>/forge
```
...
```text
CAP_RUN_LOG_DIR=/mnt/shared/<user>/forge-code/logs/20260503T043757_opencode_pnp_sink_to_counter
CAP_MCP_MAX_SESSIONS=8
PWD=/mnt/shared/<user>/forge-code
```
...
```bash
/mnt/shared/<user>/python_envs/forge/bin/python \
  -m py_compile \
  cap/mcp/server.py \
  cap/env/robocasa/skills.py \
  run_script.py \
