...
| `PI05_POLICY_LAUNCH_SCRIPT` | `/home/<user>/Project/openpi-main/.../launch_yam_policy_server_functional_grasp.sh` (env override) | `cap/config.py` | 74-77 | Path to the shell script that starts the OpenPI server |
...
bash tmux/table_bussing/table_bussing_history/launch_table_bussing_pi05.sh --pi05-server 192.0.2.77:8964
bash tmux/table_bussing/table_bussing_history/launch_table_bussing_pi05.sh --pi05-user <user>
```
...
The `pi05 tunnel` pane runs `ssh -N -L 8964:localhost:8964 <user>@<gpu-host>`, making the remote OpenPI server appear as `localhost:8964` on the robot machine. `cap_server` and `cap_agent` are started with `PI05_POLICY_SERVER=localhost:8964`.
...
bash /home/<user>/Project/openpi-main/src/openpi/serving/yam/launch_yam_policy_server.sh
```
