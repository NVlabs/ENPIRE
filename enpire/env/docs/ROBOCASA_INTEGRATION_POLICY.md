...
> **Status**: Active development on `<developer>/robocasa365-policy` branch
...
| Repo | What it provides | Needed for | Path on gpu-host |
|------|-----------------|------------|-------------------|
| **lecar-tbd** | Eval orchestrators, inference engine, docs | Both N1.5 and N1.6 | `/usr0/<user>/lecar-tbd` |
| **Isaac-GR00T** (NVIDIA) | N1.6 model code, `GrootRoboCasaEnv` wrapper, old robocasa | **N1.6 PandaOmron24 only** | `/usr0/<user>/Isaac-GR00T` |
| **Isaac-GR00T-benchmark** (robocasa-benchmark fork) | N1.5 model code, gymnasium-1.0-compatible `MultiStepWrapper` | **N1.5 RoboCasa365** (also used by N1.6 for MultiStepWrapper) | `/usr0/<user>/Isaac-GR00T-benchmark` |
| **robocasa365** (robocasa365_release branch) | 365 task classes, `RoboCasaGymEnv`, `TASK_SET_REGISTRY`, kitchen assets | **RoboCasa365 benchmark** (both N1.5 and N1.6) | `/usr0/<user>/robocasa365` |
...
3. Clones `robocasa365_release` into `/mnt/shared/<user>/robocasa365` by default.
...
export HF_HOME=/usr0/<user>/.cache/huggingface
...
| Model | Source | Path on gpu-host |
|-------|--------|-------------------|
| GR00T N1.6-3B | https://huggingface.co/nvidia/GR00T-N1.6-3B | /usr0/<user>/PretrainedModels/GR00T-N1.6-3B |
| GR00T N1.5 multitask | robocasa/robocasa365_checkpoints on HF | /usr0/<user>/PretrainedModels/gr00t_n1-5/multitask_learning/checkpoint-120000 |
...
python run_PandaOmron24.py \
    --server-host 192.0.2.5 --tasks OpenDrawer --n-eps-per-task 10
...
> `/usr0/<user>/lecar-tbd/.venv/bin/python run_PandaOmron24.py ...`
...
python run_eval_365.py --model-version n15 \
  --client-python /mnt/shared/<user>/python_envs/forge/bin/python \
  --tasks OpenDrawer --n-eps-per-task 2 --n-parallel-envs 1

# Quick test after scripts/setup_robocasa365_eval.sh:
source /mnt/shared/<user>/forge/.env.grootpool
/mnt/shared/<user>/Isaac-GR00T-benchmark/model_server_venv/bin/python run_eval_365.py \
  --model-version n15 \
  --tasks OpenDrawer --n-eps-per-task 2 --n-parallel-envs 1
...
```json
{
  ...
  "model_version": "n15",
  "model_path": "/usr0/<user>/PretrainedModels/gr00t_n1-5/...",
  "split": "pretrain",
  "n_eps_per_task": 50,
  "tasks_completed": 50,
  "tasks_total": 50,
...
> Run date: 2026-04-15, gpu-host (4× RTX PRO 6000 Blackwell 98GB)
...
3. **Root disk on gpu-host is small (49 GB)**: Always set `HF_HOME=/usr0/<user>/.cache/huggingface` before downloading models or assets. The default `~/.cache/huggingface` lives on root and will fill it.
...
SSH to gpu-host and run inside tmux:

```bash
ssh LeCAR_4xRTX6000BlackWell_97GB
tmux new -s robocasa_eval
cd /usr0/<user>/lecar-tbd/cap/saved_scripts/robocasa/policy_eval/gr00t

# Use the N1.5 server venv (has numpy, scipy, msgpack, zmq):
/usr0/<user>/Isaac-GR00T-benchmark/model_server_venv/bin/python run_eval_365.py \
  --model-version n15 \
...
ssh LeCAR_4xRTX6000BlackWell_97GB
tail -f /usr0/<user>/lecar-tbd/logs/robocasa365/n15_pretrain_<TIMESTAMP>/stdout.log

# Or check per-task results as they complete:
find /usr0/<user>/lecar-tbd/logs/robocasa365/n15_pretrain_<TIMESTAMP>/ \
...
/usr0/<user>/Isaac-GR00T-benchmark/model_server_venv/bin/python run_eval_365.py \
  ...
  --gpus 0 1 2 3 \
  --record-video \
  --log-dir /usr0/<user>/lecar-tbd/logs/robocasa365/n15_pretrain_<TIMESTAMP>
```
