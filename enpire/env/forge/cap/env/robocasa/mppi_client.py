...
class MPPINavigationClient:
    ...
    def __init__(
        ...
        if start_server:
            root = anycar_root or os.environ.get("ANYCAR_ROOT")
            if not root:
                raise RuntimeError("Set ANYCAR_ROOT or pass anycar_root to start the MPPI server")
            self._start_subprocess(root)
