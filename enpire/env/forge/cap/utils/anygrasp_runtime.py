...
from enpire.env.forge.paths import REPOSITORY_ROOT, THIRD_PARTY_ROOT

REPO_ROOT = REPOSITORY_ROOT
# The AnyGrasp SDK and its licensed binaries are deliberately not distributed
# with ENPIRE. Point this at a separately obtained SDK checkout.
VENDOR_ROOT = Path(
    os.environ.get("ANYGRASP_SDK_ROOT", THIRD_PARTY_ROOT / "anygrasp_sdk")
).expanduser().resolve()
...
def _prepare_minkowski_python_root() -> Path:
    ...
    configured_backend = os.environ.get("ANYGRASP_MINKOWSKI_BACKEND", "").strip()
    if configured_backend:
        backend_source = Path(configured_backend).expanduser().resolve()
        if not _is_elf_binary(backend_source):
            raise FileNotFoundError(
                "ANYGRASP_MINKOWSKI_BACKEND must point to a compiled ELF extension: "
                f"{backend_source}"
            )
