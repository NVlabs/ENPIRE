...
```python
# RobotState — arms are keyed by side name
state.arms               # dict[str, ArmState] — keys are arm names (e.g. "left", "right")
state.arms["right"].joint_pos   # list[float] — length depends on robot DOF
state.arms["right"].gripper_pos # float
state.arms["right"].ee_pos     # [x, y, z]  metres, world frame
state.arms["right"].ee_rpy     # [roll, pitch, yaw]  degrees
# Backward-compat shortcuts (return [] / 0.0 if arm missing):
state.left_joint_pos     # list[float]
state.right_joint_pos    # list[float]
state.left_ee_pos        # [x, y, z]  or []
state.right_ee_pos       # [x, y, z]  or []
