...
# Minimum gripper width (m) required to count as a successful grasp.
# A reading of exactly 0 (or within sensor noise of 0) means the fingers
# closed all the way — the object is not in the gripper.
...
def _base_freespace_kwargs(
    ...
    return {
        ...
        if ik_error_threshold is None
        else ik_error_threshold,
...
def birdseye_pose(
    ...
    home_view_z_offset = (
        HOME_VIEW_Z_OFFSET if home_view_z_offset is None else home_view_z_offset
    )
    left_birdeye_view_rpy = (
        LEFT_BIRDEYE_VIEW_RPY
        if left_birdeye_view_rpy is None
        else left_birdeye_view_rpy
    )
    right_birdeye_view_rpy = (
        RIGHT_BIRDEYE_VIEW_RPY
        if right_birdeye_view_rpy is None
        else right_birdeye_view_rpy
    )
    if side == "left":
        return (
...
def _go_local_pose(**config):
    return freespace_move(
        left_target_pos=list(
            config.get("local_left_target_pos", LOCAL_LEFT_TARGET_POS)
        ),
        left_target_rpy=list(
            config.get("local_left_target_rpy", LOCAL_LEFT_TARGET_RPY)
        ),
        right_target_pos=list(
            config.get("local_right_target_pos", LOCAL_RIGHT_TARGET_POS)
        ),
        right_target_rpy=list(
            config.get("local_right_target_rpy", LOCAL_RIGHT_TARGET_RPY)
        ),
        left_gripper_target_width=1.0,
        right_gripper_target_width=1.0,
        planning_speed=config.get("local_planning_speed", LOCAL_PLANNING_SPEED),
        ik_error_threshold=config.get(
...
def choose_arm(xyz):
    ...
    left_dist = float(
        np.linalg.norm(obj_pos - np.asarray(state.left_ee_pos, dtype=float))
    )
    right_dist = float(
        np.linalg.norm(obj_pos - np.asarray(state.right_ee_pos, dtype=float))
    )
...
def _gripper_command_kwargs(config, action):
    kwargs = {}
    vel_limit = config.get(f"gripper_{action}_vel_limit")
    torque_limit = config.get(f"gripper_{action}_torque_limit")
    ...
    **config,
    ...
    try:
        if trajectory_cache_key is not None:
            freespace_move(
                preview_only=False, trajectory_cache_key=trajectory_cache_key
            )
            return True
        kwargs = _base_freespace_kwargs(
            preview_only=False,
...
def _select_best_grasp(grasps, side, label="grasp", **config):
    ...
    if best is not None and getattr(best, "motion_plan_error", True) is False:
        source_grasp = None
        source_index = getattr(best, "source_index", None)
        if source_index is not None:
            try:
                source_grasp = grasps[int(source_index)]
            except Exception:
                source_grasp = None
        SelectedGrasp = collections.namedtuple(
            "SelectedGrasp",
            ["position", "rpy", "score", "width", "trajectory_cache_key", "bbox_result"],
        )
        selected = SelectedGrasp(
            ...
            bbox_result=(
                getattr(source_grasp, "bbox_result", None)
                if source_grasp is not None
                else getattr(best, "bbox_result", None)
            ),
        )
        print(
...
def _execute_grasp(side, grasp, label="grasp", **config):
    open_kwargs = _gripper_command_kwargs(config, "open")
    close_kwargs = _gripper_command_kwargs(config, "close")
    adjust_fn = config.get("selected_grasp_adjust_fn")
    execute_fn = config.get("selected_grasp_execute_fn")
    if open_kwargs or close_kwargs:
        print(
            f"  Gripper tuning [{side}]: "
            f"open={open_kwargs or '{}'} close={close_kwargs or '{}'}"
        )
    if callable(adjust_fn):
        adjusted_grasp = adjust_fn(grasp, side=side, label=label)
        if adjusted_grasp is not None:
            grasp = adjusted_grasp
...
def pick_object(
    object_name, camera="top", grasp_mode="anygrasp", max_attempts=None, **kwargs
):
    ...
    for attempt in range(1, attempts + 1):
        ...
        if camera in ("left", "right"):
            ...
            grasps = _grasp_candidates(
                object_name, camera, grasp_mode, max_grasps, **kwargs
            )
            if not grasps:
            ...
            side = choose_arm(
                grasps[0].position
                if hasattr(grasps[0], "position")
                else grasps[0]["position"]
            )
            label = grasp_mode
        ...
        while True:
            ...
            print(
                "  Direct grasp planning from current pose failed; going home and retrying"
            )
...
def _detect_bundlesdf_pose(object_name, camera=BUNDLESDF_CAMERA):
    ...
    if not dets:
        raise RuntimeError(
            f"No BundleSDF one-shot detections found for {object_name!r}"
        )
    det = dets[0]
    if not getattr(det, "position_3d", None):
        raise RuntimeError(
            f"BundleSDF detection for {object_name!r} did not return position_3d"
        )
    return det
...
def estimate_drop(target_name, z_offset=None, camera=None, target_drop_z_offsets=None):
    ...
    offsets = (
        ...
        if target_drop_z_offsets is None
        else target_drop_z_offsets
    )
    offset = float(
        offsets.get(target_name, DEFAULT_TARGET_DROP_Z_OFFSET)
        if z_offset is None
        else z_offset
    )
...
def _try_place_without_failure_release(
    side, object_name, target_name, transport_rpy, **kwargs
):
    try:
        return place_object(
            side,
...
def _place_with_recovery(side, object_name, target_name, transport_rpy, **kwargs):
    if _try_place_without_failure_release(
        side, object_name, target_name, transport_rpy, **kwargs
    ):
        return True

    print(
        "  Place planning failed; keeping gripper closed and retrying from BirdEyeView"
    )
    ...
    print(
        "  Place planning still failed; keeping gripper closed and retrying from home"
    )
