...
SAM3 calls to the TRT server on :6868) instead of the PyTorch _compute_rew_rgb.py.

Run the RTT/SAM3-only launch stack before launching this script:
...
# Handover-pose move lives in _move_to_rew_pose.py (single source of truth, shared with PyTorch variant).
...
print(f"[reward-trt] loop done after {time.time()-t_start:.0f}s; render pool "
