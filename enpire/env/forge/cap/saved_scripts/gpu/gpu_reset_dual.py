...
each slot. This avoids tearing down and recreating the real robot/camera env
between the two unplug operations.
"""
...
def _run_gpu_reset_slot(slot: int, *, go_home_on_start: bool) -> dict:
    ...
    print(
        "[gpu_reset_dual] Running gpu_reset.py for "
    ...
    try:
        ...
        namespace.update(
            {
                ...
                "__name__": f"gpu_reset_slot_{int(slot)}",
            }
        )
        ...
        if not success:
            raise RuntimeError(f"slot {int(slot)} gpu_reset.py did not report success: {info}")
        return info
...
def main():
    ...
    print("[gpu_reset_dual] Complete: slot 3 and slot 1 reset succeeded.")
