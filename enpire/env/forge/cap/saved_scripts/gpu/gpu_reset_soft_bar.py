...
Soft-bar variant:
  - keeps the original slot-3 pre-descent refresh behavior unchanged,
  - keeps the original aux-camera world-pose hover behavior,
  - uses a separate softer close for the metal-bar loosening grasp,
  - uses a shorter slot-3 metal-bar loosen arc so slot 3 does not unplug
    before the center regrasp,
  - opens/releases the bar grasp from a slightly higher support pose.
...
SLOT3_BAR_GRASP_Z_EXTRA_M = _env_float(
    "GPU_UNPLUG_SLOT3_BAR_GRASP_Z_EXTRA_M",
    0.001,
)
BAR_LOOSEN_LIFT_M = _env_float("GPU_UNPLUG_BAR_LOOSEN_LIFT_M", 0.028)
SLOT3_BAR_LOOSEN_LIFT_M = _env_float(
    "GPU_UNPLUG_SLOT3_BAR_LOOSEN_LIFT_M",
    0.014,
)
BAR_PULL_FORWARD_X_M = _env_float("GPU_UNPLUG_BAR_PULL_FORWARD_X_M", 0.018)
SLOT3_BAR_PULL_FORWARD_X_M = _env_float(
    "GPU_UNPLUG_SLOT3_BAR_PULL_FORWARD_X_M",
    0.006,
)
...
BAR_CLOSE_VEL_LIMIT = _env_float(
    "GPU_UNPLUG_BAR_CLOSE_VEL_LIMIT",
    float(LEFT_CLOSE_VEL_LIMIT),
)
BAR_CLOSE_TORQUE_LIMIT = _env_float(
    "GPU_UNPLUG_BAR_CLOSE_TORQUE_LIMIT",
    0.70,
)
SLOT3_BAR_CLOSE_TORQUE_LIMIT = _env_float(
    "GPU_UNPLUG_SLOT3_BAR_CLOSE_TORQUE_LIMIT",
    0.65,
)
...
def _bar_close_torque_limit():
    if int(TARGET_SOCKET_NUMBER) == 3:
        return float(SLOT3_BAR_CLOSE_TORQUE_LIMIT)
    return float(BAR_CLOSE_TORQUE_LIMIT)


def _bar_grasp_z_extra_m():
    if int(TARGET_SOCKET_NUMBER) == 3:
        return float(SLOT3_BAR_GRASP_Z_EXTRA_M)
    return 0.0


def _bar_loosen_lift_m():
    if int(TARGET_SOCKET_NUMBER) == 3:
        return float(SLOT3_BAR_LOOSEN_LIFT_M)
    return float(BAR_LOOSEN_LIFT_M)


def _bar_pull_forward_x_m():
    if int(TARGET_SOCKET_NUMBER) == 3:
        return float(SLOT3_BAR_PULL_FORWARD_X_M)
    return float(BAR_PULL_FORWARD_X_M)
...
def _build_short_lift_pos(scene, grasp_pos):
    hover_z = float(scene["hover_z"])
    loosen_lift_m = float(_bar_loosen_lift_m())
    lift_z = max(
        float(grasp_pos[2]) + loosen_lift_m,
        min(float(hover_z), float(grasp_pos[2]) + loosen_lift_m),
    )
...
def _build_bar_arc_lift_waypoints(scene, grasp_pos):
    ...
    print(
        ...
        f"forward_x_m={pull_forward_x_m:.4f} "
...
def _firm_close_left_gripper(
    *,
    reclamp_attempts=None,
    label_prefix="close left gripper",
    vel_limit=None,
    torque_limit=None,
):
    ...
    close_vel_limit = float(LEFT_CLOSE_VEL_LIMIT if vel_limit is None else vel_limit)
    close_torque_limit = float(LEFT_CLOSE_TORQUE_LIMIT if torque_limit is None else torque_limit)
    print(
        f"[gpu_unplug] {label_prefix}: "
        f"vel_limit={close_vel_limit:.3f} "
        f"torque_limit={close_torque_limit:.3f}"
    )
    gh._close_gripper(
        "left",
        vel_limit=close_vel_limit,
        torque_limit=close_torque_limit,
    )
...
def main():
    ...
    print(
        ...
        f"slot3_bar_grasp_z_extra_m={float(SLOT3_BAR_GRASP_Z_EXTRA_M):.4f} "
        f"active_bar_grasp_z_extra_m={float(_bar_grasp_z_extra_m()):.4f} "
        f"bar_loosen_lift_m={float(BAR_LOOSEN_LIFT_M):.4f} "
        f"slot3_bar_loosen_lift_m={float(SLOT3_BAR_LOOSEN_LIFT_M):.4f} "
        f"active_bar_loosen_lift_m={float(_bar_loosen_lift_m()):.4f} "
        f"bar_pull_forward_x_m={float(BAR_PULL_FORWARD_X_M):.4f} "
        f"slot3_bar_pull_forward_x_m={float(SLOT3_BAR_PULL_FORWARD_X_M):.4f} "
        f"active_bar_pull_forward_x_m={float(_bar_pull_forward_x_m()):.4f} "
        ...
        f"bar_close_vel={float(BAR_CLOSE_VEL_LIMIT):.3f} "
        f"bar_close_torque={float(BAR_CLOSE_TORQUE_LIMIT):.3f} "
        f"slot3_bar_close_torque={float(SLOT3_BAR_CLOSE_TORQUE_LIMIT):.3f} "
        f"active_bar_close_torque={float(_bar_close_torque_limit()):.3f} "
    ...
    if bar_current_scene is not None:
        ...
        grasp_z_extra_m = float(_bar_grasp_z_extra_m())
        if abs(grasp_z_extra_m) > 1e-9:
            bar_grasp_pos[2] = float(bar_grasp_pos[2]) + grasp_z_extra_m
            print(
                "[gpu_unplug] bar-stage slot-specific grasp z trim: "
                f"target_socket={int(TARGET_SOCKET_NUMBER)} "
                f"extra_z_m={grasp_z_extra_m:.4f} "
                f"bar_grasp_pos={[round(float(v), 4) for v in bar_grasp_pos]}"
            )
        _guided_left_move(
            bar_grasp_pos,
            hover_rpy,
        ...
        _firm_close_left_gripper(
            ...
            vel_limit=BAR_CLOSE_VEL_LIMIT,
            torque_limit=_bar_close_torque_limit(),
        )
